...
- **凭证校验已实现，但默认不启用。** registry 现在校验 `hello` 里的 `credential`，用常量时间比较；期望值来自插件配置的 `credential` 字段，或 `ctx.credentials` 解析的 `nodeCredential` 引用（前者优先）。两者都没配时**完全不校验**——这是为兼容旧部署保留的默认，host 会在每次注册时把这件事打在 stderr 上，避免出现"静默敞开"。

  仍缺的：没有注册（enrollment）流程，凭证由安装脚本生成或由你提供；没有轮换机制（改配置 + 重启，旧节点会被拒到更新为止）；没有逐节点身份——校验回答的是"是不是同一个共享密钥"，不是"你是哪台机器"。见 `SECURITY.md`。
